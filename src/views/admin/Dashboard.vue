...
<b-container fluid >
  ...
  <b-row align-v="center" class="mx-auto" >
    <Card v-for="(i,idx) in categories" :class={hideCard:hide} :key="idx" :v="i.color" :header="i.name" :content="frameContent(i.status)"/>
 </b-row>
  ...
  <IssuesCategories :categories="categories" v-slot:default="slotProps">
    ...
    <IssuesList  idPre="slot1" :list="getIssues" v-slot:default="innerSlotProps" >
    <div v-if="filterIssues(innerSlotProps.data.issues,slotProps.status).length>0">
    <p>{{`${innerSlotProps.data.firstName} ${innerSlotProps.data.lastName}`}}</p>
      <InnerList  :idx="innerSlotProps.idx" >
       ...
       <div  @click="initResponse(i,innerSlotProps.data)" :key="idx" v-for="(i,idx) in filterIssues(innerSlotProps.data.issues,slotProps.status)">
         <b-card>
 ...
 <RespondIssue id='respond' title='Respond to issue' :cIssue="cIssue" :cPatient="cPatient"/> 
</b-container>
</template>
...
export default {
  ...
  initResponse(e,i){
    this.cIssue=e;
    this.cPatient=i;
    if(this.cIssue&&this.cPatient)
    this.$bvModal.show('respond')
  },
  frameContent(i){
