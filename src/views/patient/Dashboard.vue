...
<b-container fluid >
  ...
  <b-row align-v="center" class="mx-auto" >
    <Card  :class={hideCard:hide} v-b-modal.modal-1  v="secondary" header="New" content="Click to report new issue"/>
  ...
  <IssuesCategories :categories="categories" v-slot:default="slotProps">
    ...
    <IssuesList :list="getIssuesByStatus(slotProps.status)" v-slot:default="innerSlotProps" >
      ...
      <InnerList idPre="slot" :idx="innerSlotProps.idx" >
        ...
        <b-button v-if="innerSlotProps.data.response" @click="markAsResolved(innerSlotProps.data)" variant="outline-primary">Mark as resolved</b-button>
      </InnerList>
   </IssuesList>
   <div v-if="getIssuesByStatus(slotProps.status).length===0">{{nodata}}</div>
  </IssuesCategories>
   </b-row>
